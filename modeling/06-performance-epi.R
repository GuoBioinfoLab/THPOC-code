...
fn_get_epi_task <- function(.list, .w) {
  .w@colData %>%
    as.data.frame() %>%
  ...
  .list$panel$samples <- list(
    "Epi" = .list.panel.samples[.benign_epi],
    "Non-epi" = .list.panel.samples[.benign_nepi]
  )

  .list.ca125.samples <- .list$ca125$samples %>% purrr::reduce(c)
  .list$ca125$samples <- list(
    "Epi" = .list.ca125.samples[.benign_epi],
    "Non-epi" = .list.ca125.samples[.benign_nepi]
  )
...
save.image(file = "data/rda/06-performance-epi.rda")
